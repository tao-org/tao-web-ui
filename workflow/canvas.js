    var addNewNode = function (x, y, dna) {
			var nodeID = jsPlumbUtil.uuid();
			dna.nodeID = nodeID;
			dna.nodePorts = {
                    "in":[
							{"type":"out","label":"Nod iesire ...","datatype":"RASTER_IMG"},
							{"type":"out","label":"Nod iesire ...","datatype":"RASTER_IMG"},
							{"type":"out","label":"Nod iesire ...","datatype":"RASTER_IMG"}
					],
                    "out":[
							{"type":"in","label":"Nod iesire ...","datatype":"RASTER_IMG"},
							{"type":"in","label":"Nod iesire ...","datatype":"RASTER_IMG"},
							{"type":"in","label":"Nod iesire ...","datatype":"RASTER_IMG"},
							{"type":"in","label":"Nod iesire ...","datatype":"RASTER_IMG"},
							{"type":"in","label":"Nod iesire ...","datatype":"RASTER_IMG"}
	...
	var newNodeBody = function(x, y, dna) {
		var completeness = 30;
		var maxPorts = Math.max(dna.nodePorts.in.length, dna.nodePorts.out.length);
		...
		for (i = 0; i < maxPorts; i++) {
			...
			if(dna.nodePorts.out[i]) innerHTML += "<div id=\"p-out-"+i+"-"+dna.nodeID+"\" class=\"n-p-o-wrapp\"><div class=\"n-p-o\"></div></div>";
			if(dna.nodePorts.in[i]) innerHTML += "<div id=\"p-in-"+i+"-"+dna.nodeID+"\" class=\"n-p-i-wrapp\"><div class=\"n-p-i\"></div></div>";
			innerHTML += "</div>";
		}
...
//		
		...
		for (i = 0; i < maxPorts; i++) {
			if(dna.nodePorts.out[i]) {
				var elPort = document.getElementById("p-out-"+i+"-"+dna.nodeID);
				initPort(elPort, "out");
			}
			if(dna.nodePorts.in[i]) {
				var elPort = document.getElementById("p-in-"+i+"-"+dna.nodeID);
				initPort(elPort, "in");
			}
		}
